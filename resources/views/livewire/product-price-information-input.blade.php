<label class="flex flex-col tracking-wider">Price
    <div class="flex">
        <input wire:input="calculateDiscountedPrice" @if($currentVariant)name="variants[][price]" @endif wire:model="price" id="price" type="number" class="bg-white/10 border-l-0 border-r-0 border-t-0 border-b-2 ml-2 h-9 w-36" step="0.01">
        <select wire:change="setCurrency" @if($currentVariant)name="variants[][currency]" @endif wire:model="currency" id="currency-select" class="bg-white/10 border-l-0 border-r-0 border-t-0 border-b-2 ml-2 h-9 text-xs">
