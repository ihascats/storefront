  ...
  <div class="text-white p-4 max-w-7xl mx-auto">
    <form  method="POST" action="{{ route('products.store') }}" class="flex flex-col gap-3">
      ...
      <div class="dropdown pl-2">
          <button id="categoriesDropdownButton" type="button" class="py-2 px-4 bg-slate-800 text-gray-300 rounded-md border border-slate-600 hover:bg-slate-600 focus:outline-none focus:bg-slate-800 w-full">
              Select Categories
          </button>
          <ul id="categoriesDropdown" class="bg-slate-800 border border-slate-600 py-1 w-full rounded-md shadow-lg hidden">
              <label for="new_category" class="relative flex gap-2 items-center">
                  <input type="text" id="new_category" placeholder="New Category" class="inline float-left bg-white/10 border-l-0 border-r-0 border-t-0 border-b-2 ml-2 h-7">
                  <button id="addCategory" type="button" class="bg-orange-600 rounded-md px-4 h-7">Add</button>
              </label>
              @if($categories)
